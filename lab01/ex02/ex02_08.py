...
so_chia_cho_5= [so for so in so_nhi_phan_list if so_chia_cho_5(so)]
if len(so_chia_cho_5) > 0:
    ket_qua = ','.join(so_chia_cho_5)
    print("Cac so nhi phan chia het cho 5 la : ", ket_qua)
else:
    print("Khong co so nhi phan nao chia het cho 5 tron chuoi da nhap.")